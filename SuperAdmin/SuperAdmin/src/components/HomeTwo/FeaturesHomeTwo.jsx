import React from 'react';
// import featureThumb from '../../assets/images/features-thumb-2.png';
// import featureThumb from '../../assets/images/slider-3.jpg';
// import featureThumb from '../../assets/images/howitworks.png';
import featureThumb from '../../assets/images/hero-thumb-9.png';

import shape13 from '../../assets/images/shape/shape-13.png';
import shape14 from '../../assets/images/shape/shape-14.png';
import shape15 from '../../assets/images/shape/shape-15.png';

function FeaturesHomeTwo() {
    return (
        <>
            <section className="appie-features-area-2 pt-90 pb-100" id="features">
                <div className="container">
                    <div className="row justify-content-center">
                        <div className="col-lg-12">
                            <div className="appie-section-title appie-section-title-2 text-center">
                                <h3 className="appie-title">
                                How it Works <br />
                                    
                                </h3>
                                <p>
                                Lets Join ! Become Our Family Member
                                </p>
                            </div>
                        </div>
                    </div>
                    <div className="row mt-30 align-items-center">
                        <div className="col-lg-6">
                            <div className="appie-features-boxes">
                                <div className="appie-features-box-item">
                                    <h5 className='text-white mb-2'>Step 01</h5>
                                    <h4 className="title">Create An Account</h4>
                                    <p>Create Own Team With Your Super Distributer, Distributer And Retailer Accounts.</p>
                                </div>
                                <div className="appie-features-box-item item-2">
                                   <h5 className='text-white mb-2'>Step 02</h5>
                                    <h4 className="title">Buy & Distribute Coupon</h4>
                                    <p>Buy Coupon And Distribute Coupons To Your Team With Realtime Coupon Updation.</p>
                                </div>
                                <div className="appie-features-box-item item-3">
                                  <h5 className='text-white mb-2'>Step 03</h5>
                                    <h4 className="title">Check History</h4>
                                    <p>Get The Realtime Coupon Buying Data, Distributed Data And Joining Data.</p>
                                </div>
                            </div>
                        </div>
                        <div className="col-lg-6">
                            <div
                                className="appie-features-thumb wow animated fadeInRight"
                                data-wow-duration="2000ms"
                                data-wow-delay="200ms"
                            >
                                <img src={featureThumb} alt="" />
                                {/* <img src={heroThumb} alt="" /> */}
                            </div>
                        </div>
                    </div>
                </div>
                <div className="features-shape-1">
                    <img src={shape15} alt="" />
                </div>
                <div className="features-shape-2">
                    <img src={shape14} alt="" />
                </div>
                <div className="features-shape-3">
                    <img src={shape13} alt="" />
                </div>
            </section>
        </>
    );
}

export default FeaturesHomeTwo;

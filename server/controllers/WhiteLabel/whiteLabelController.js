...
const idTransfer = (req, res) => {
  try {
    ...
    // ✅ Determine `transferType` based on role
    // Backend me `transferType` ki mapping sahi karna zaroori hai
    if (transferType?.toLowerCase() === "distributor") {
      transferType = "remaining_distributor_id"; // ✅ Super Distributor → Distributor ID
    } else if (transferType?.toLowerCase() === "retailer") {
      transferType = "remaining_retailer_id"; // ✅ Distributor → Retailer ID
    } else {
      return res.status(400).json({
        success: false,
        ...
        db.query(updateSenderQuery, [newSenderIdBalance, senderId], (error) => {
          ...
          db.query(
            ...
            (error) => {
              ...
              INSERT INTO id_transfer_log (senderId, receiverId, transferType, no_of_ids)
              VALUES (?, ?, ?, ?)
            `;
              ...
              db.query(
                insertLogQuery,
                [senderId, receiverId, mappedTransferType, transferCount],
                (error) => {
                  if (error) {
...
const getShareIdsSummary = async (req, res) => {
  ...
  try {
    const sql = "SELECT * FROM id_transfer_log WHERE senderId = ? ";
    ...
    db.query(sql, [userId], (err, result) => {
      ...
      return res.status(200).json({
        success: true,
        data: result, // ✅ Saari rows return kar raha hai
        message: "Data fetched successfully",
      });
    });
  } catch (error) {

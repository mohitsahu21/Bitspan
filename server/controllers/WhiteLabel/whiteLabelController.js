...
const getApplyOfflineFormById = (req, res) => {
  ...
  const sql = `
    ...
    WHERE ur.white_Lable = ? AND ur.userType = 'Retailer'
...
//   const baseUrl = `process.env.domain/uploads/`;
...
const getSpecificUserTransactions = (req, res) => {
  ...
  const sql = `
    ...
    WHERE ur.white_Lable = ? AND ur.userType = 'Retailer' ORDER BY id DESC
  `;
...
const getEDistrictHistory = (req, res) => {
  ...
  const sql = `
    ...
    ON ur.UserId = t.user_id 
    WHERE ur.white_Lable= ? AND ur.userType = 'Retailer'
    AND t.samagar = 'Ekyc' ORDER BY id DESC
  `;
...
const getWlOnlineRecharges = (req, res) => {
  const { userId } = req.params;
...
const getActiveUsers = (req, res) => {
  ...
  try {
    const sql = `
      ...
      ON ur.UserId = t.UserId 
      WHERE ur.white_lable = ? 
      AND (ur.userType = 'Retailer' OR ur.userType = 'Distributor' OR ur.userType = 'superDistributor')
      AND t.Status = 'Active' ORDER BY id DESC
    `;
  ...
  try {
    const sql = `
      ...
      ON ur.UserId = t.UserId 
       WHERE ur.white_lable = ? 
      AND (ur.userType = 'Retailer' OR ur.userType = 'Distributor' OR ur.userType = 'superDistributor')
      AND t.Status = 'Deactive' ORDER BY id DESC
    `;
...
const getWhiteLabelUsersData = (req, res) => {
  const { userId } = req.params;
  ...
  try {
    const sql = `
      ...
      ON ur.UserId = t.UserId 
     WHERE ur.white_lable = ? 
      AND (ur.userType = 'Retailer' OR ur.userType = 'Distributor' OR ur.userType = 'superDistributor') ORDER BY id DESC
    `;
...
const getPendingKycUsers = (req, res) => {
  ...
  const sql = `
    ...
    AND Status = 'Pending' 
    AND payment_status = 'Complete' ORDER BY id DESC
  `;
  ...
  db.query(sql, [userId], (err, result) => {
    ...
    if (result.length === 0) {
      return res.status(200).json({
        success: false,
        data: [],
        message: "No pending records with complete payment found",
      });
    }
...
const getWhiteLabeljoinedData = (req, res) => {
  const { userId } = req.params; // This is coming from Redux in the frontend
  ...
  // SQL query to fetch users where created_By_User_Id = userId
  // const sql = `
  //   SELECT
  //     id, UserId, UserName, role, ContactNo, Email, PanCardNumber,
  //     AadharNumber, BusinessName, City, State, PinCode,
  //     AadharFront, AadharBack, PanCardFront, profileImage,
  //     White_Label_Website_URL, Status, payment_status, package_Id,
  //     created_By_User_Id, created_By_User_Role, created_By_Website,
  //     twostep_pin, remaining_whitelable_id, remaining_superDistributor_id,
  //     remaining_distributor_id, remaining_retailer_id, Note, CreateAt
  //   FROM userprofile
  //   WHERE created_By_User_Id = ?`;
  ...
  const sql = `
     ...
     *
    FROM userprofile 
    WHERE created_By_User_Id = ? AND source = "Created by WhiteLabel Dashboard" ORDER BY id DESC`;
  ...
  db.query(sql, [userId], (err, result) => {
    ...
    if (result.length === 0) {
      return res.status(200).json({
        success: false,
        data: [],
        message: "No users found for the given created_By_User_Id",
      });
    }
...
const getAllComplaintsData = (req, res) => {
  try {
    const userId = req.params.userId;
    const selectQuery = "SELECT * FROM complaindata WHERE userID = ? ORDER BY id DESC";
...
const VerifyOtpEmail = async (email, otp) => {
  console.log(`ðŸ“© Sending Email to: ${email}, OTP: ${otp}`);
  try {
    const mailOptions = {
...
const idTransfer = (req, res) => {
  try {
    const { senderId, receiverId, no_of_ids } = req.body;
    let { transferType } = req.body;
    ...
    // Validate `no_of_ids`
    if (!no_of_ids || isNaN(parseInt(no_of_ids)) || parseInt(no_of_ids) <= 0) {
      return res.status(400).json({
        success: false,
        error: "Invalid or missing number of IDs to transfer",
      });
    }

    const transferCount = parseInt(no_of_ids);

    // âœ… Convert role to lowercase to avoid case sensitivity issues
    const roleLowerCase = transferType?.toLowerCase();

    // âœ… Determine `transferType` based on role
    // Backend me `transferType` ki mapping sahi karna zaroori hai
    if (transferType?.toLowerCase() === "distributor") {
      transferType = "remaining_distributor_id"; // âœ… Super Distributor â†’ Distributor ID
    } else if (transferType?.toLowerCase() === "retailer") {
      transferType = "remaining_retailer_id"; // âœ… Distributor â†’ Retailer ID
    } else {
      return res.status(400).json({
        success: false,
    ...
    console.log("Final transferType in Backend:", transferType);

    // Validate `transferType` (Konsa ID type transfer kar rahe hain)
    ...
    // Mapping the transferType for log
    const mappedTransferType =
      transferType === "remaining_distributor_id"
        ? "Distributor Id"
        : "Retailer Id";

    console.log(transferType);

    // Query to get the sender's current remaining IDs
    const getSenderIdsQuery = `SELECT ${transferType} FROM userprofile WHERE userId = ?`;

    db.query(getSenderIdsQuery, [senderId], (error, senderResults) => {
      if (error) {
        console.error("Error fetching sender ID balance:", error);
        return res.status(500).json({
          success: false,
          error: "Failed to fetch sender ID balance",
        });
      }
      console.log(senderResults[0]);
      const senderIdBalance = senderResults.length
        ? parseInt(senderResults[0][transferType])
        : 0;

      // Check if sender has sufficient IDs to transfer
      if (senderIdBalance < transferCount) {
        return res.status(400).json({
          success: false,
          error: "Insufficient IDs for transfer",
        });
      }

      const newSenderIdBalance = senderIdBalance - transferCount;

      // Query to get the receiver's current remaining IDs
      db.query(getSenderIdsQuery, [receiverId], (error, receiverResults) => {
        if (error) {
          console.error("Error fetching receiver ID balance:", error);
          return res.status(500).json({
            success: false,
            error: "Failed to fetch receiver ID balance",
          });
        }

        const receiverIdBalance = receiverResults.length
          ? parseInt(receiverResults[0][transferType])
          : 0;
        const newReceiverIdBalance = receiverIdBalance + transferCount;

        // SQL queries to update sender's and receiver's ID balances
        const updateSenderQuery = `UPDATE userprofile SET ${transferType} = ? WHERE userId = ?`;
        const updateReceiverQuery = `UPDATE userprofile SET ${transferType} = ? WHERE userId = ?`;

        // Execute transaction queries for sender and receiver
        db.query(updateSenderQuery, [newSenderIdBalance, senderId], (error) => {
          if (error) {
            console.error("Error updating sender ID balance:", error);
            return res.status(500).json({
              success: false,
              error: "Failed to update sender ID balance",
            });
          }

          db.query(
            updateReceiverQuery,
            [newReceiverIdBalance, receiverId],
            (error) => {
              if (error) {
                console.error("Error updating receiver ID balance:", error);
                return res.status(500).json({
                  success: false,
                  error: "Failed to update receiver ID ",
                });
              }

              const insertLogQuery = `
              INSERT INTO id_transfer_log (senderId, receiverId, transferType, no_of_ids , transferDate)
              VALUES (?, ?, ?, ?,?)
            `;
              ...
              db.query(
                insertLogQuery,
                [senderId, receiverId, mappedTransferType, transferCount,transactionDate],
                (error) => {
                  if (error) {
                    console.error("Error inserting transfer log:", error);
                    return res.status(500).json({
                      success: false,
...
const getShareIdsSummary = async (req, res) => {
  ...
  try {
    const sql = "SELECT * FROM id_transfer_log WHERE senderId = ? ORDER BY id DESC";
...
const buyId = (req, res) => {
  ...
  db.query(getClosingBalanceQuery, [userId], (error, results) => {
    ...
    // Query to insert data into userid_bought_summary table
    const insertQuery = `
      INSERT INTO userid_bought_summary (userId, number_of_userId, userId_amount, userId_type, orderId, transactionId, bought_date, total_amount, payment_method,status)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
...
const getWhitelabelSettings = (req, res) => {
  ...
  try {
    const sql = `SELECT * 
    FROM white_label_website_setting
     WHERE whiteLabel_id = ?
     `;
    ...
    db.query(sql, [userId], (err, result) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ success: false, error: "Error white_label_website_setting" });
      } else {
        // Check if the result is empty
        if (result.length === 0) {
          return res.status(200).json({
...
const UpdateGenralSetting = (req, res) => {
  try {
    ...
    if (!whiteLabel_id) {
      return res
        .status(400)
        .json({ success: false, error: "whiteLabel_id is required" });
    }
    ...
    const sql = `UPDATE white_label_website_setting SET
        Home_Page_1st_Paragraph = ? ,
          Home_Page_2nd_Paragraph = ?,
      Email_Id = ?,
      Calling_No = ?,
    ...
    db.query(sql, values, (error, results) => {
      if (error) {
        ...
        return res
          .status(500)
          .json({ success: false, error: "Failed to update the package" });
      }
...
const UpdateWLWebsiteJoiningPrice = (req, res) => {
  try {
    const {
      whiteLabel_id,
    ...
    db.query(sql, values, (error, results) => {
      if (error) {
        ...
        return res.status(500).json({
          success: false,
          error: "Failed to UpdateWLWebsiteJoiningPrice",
        });
      }
      ...
      return res.status(200).json({
        success: true,
        message: "UpdateWLWebsiteJoiningPrice successfully",
      });
    });
  } catch (error) {
    console.error("Unexpected error:", error);
  ...
  try {
    ...
    if (!userId) {
      return res.status(400).json({
        success: false,
        message: "User ID is required.",
        error: "Missing userId parameter in request.",
      });
    }
    ...
    const query = `
      ...
      AND created_By_User_Role = 'whiteLabel' ORDER BY id DESC
    `;
    ...
    db.query(query, [userId], (err, results) => {
      ...
      // 🔹 Yeh log check karega ki results me kis user ki ID aa rahi hai
      results.forEach((user) => {
        console.log(
          `UserId: ${user.UserId}, Created_By_User_Id: ${user.created_By_User_Id}`
        );
      });
      ...
      if (results.length === 0) {
        return res.status(404).json({
          success: false,
          message: `No users found for created_By_User_Id: ${userId}.`,
          error: "User data not available in the database.",
        });
      }

      res.status(200).json({
        success: true,
        message: `Users created by userId ${userId} retrieved successfully.`,
...
const WLGetContactUs = async (req, res) => {
  try {
    const { userId } = req.params;
    ...
    const sql = "SELECT * FROM wl_contactus WHERE userId = ? ORDER BY id DESC";

    console.log("🔍 Executing Query:", sql);
    console.log("📌 Query Parameter (userId):", userId);
    ...
    db.query(sql, [userId], (err, results) => {
      ...
      // 🛑 No records found
      if (!results || results.length === 0) {
        console.warn(`⚠️ No records found for userId: ${userId}`);
        return res.status(404).json({
          success: false,
          message: `No contact records found for userId: ${userId}.`,
          error: "No matching records in the database.",
        });
      }
      ...
      res.status(200).json({
        success: true,
        message: `Successfully retrieved ${results.length} contact records for userId: ${userId}.`,

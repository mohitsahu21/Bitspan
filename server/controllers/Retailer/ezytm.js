...
const getRechargeStatus = (req, res) => {
  ...
  makeRechargeRequest(endpoint, {
    ...
    .then((data) => {
      res.json({ message: "Recharge status checked successfully", data });
    })
    .catch((error) => {
      res.status(500).send("Error fetching data from client API");
    });
...
//       .then(({ rechargeData, orderId }) => {
//         const updateQuery =
//           "UPDATE recharges SET opcode = ?, status = ?, transaction_id = ?, opid = ?, dr_amount = ?, errorcode = ?, orderid = ?, updated_at = ? WHERE orderid = ?";
//         const updateValues = [
//           rechargeData.opcode || "",
//           rechargeData.status,
//           rechargeData.txid || "",

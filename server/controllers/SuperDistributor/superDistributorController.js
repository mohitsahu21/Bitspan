...
const getApplyOfflineFormById = (req, res) => {
  ...
  const sql = `
    ...
    WHERE ur.superDistributor = ? AND ur.userType = 'Retailer'
...
const getActiveUsers = (req, res) => {
  ...
  try {
    const sql = `
      ...
      ON ur.UserId = t.UserId 
      WHERE ur.superDistributor = ? 
     AND ur.userType = 'Distributor'
      AND t.Status = 'Active'
    `;
...
const getDeactiveUsers = (req, res) => {
  ...
  try {
    const sql = `
      ...
      ON ur.UserId = t.UserId 
      WHERE ur.superDistributor = ? 
      AND (ur.userType = 'Retailer' OR ur.userType = 'Distributor')
...
const getAllComplaintsData = (req, res) => {
  try {
    const userId = req.params.userId;
    const selectQuery = "SELECT * FROM complaindata WHERE userID = ? ORDER BY createdAt DESC";
    ...
    db.query(
      ...
      (err, result) => {
        if (err) {
          ...
          return res
            .status(500)
            .json({ message: "Failed to save the purchase.",err });
        }

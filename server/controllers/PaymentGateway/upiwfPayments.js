...
const BuyUserIdUsingPGVerify = async (req, res) => {
  ...
  try {
    ...
    const [dbData] = await new Promise((resolve, reject) => {
      db.query(
        "SELECT * FROM userid_bought_summary WHERE orderId = ?",
        [clientTxnId],
        (err, results) => {
          if (err) reject(err);
          resolve(results);
    ...
    if (response.data.status === "Success") {
      ...
      if (dbData.status === "Pending") {
        ...
        await new Promise((resolve, reject) => {
          db.query(
            "UPDATE userid_bought_summary SET status = ? , transactionId = ?  WHERE orderId = ?",
            ["Success", utr, clientTxnId],
            (err) => {
              if (err) reject(err);
              resolve();
            }
                  ...
                  try {
                    ...
                    if (response.data && response.data.status === "Success") {
                      return res.status(200).json({
                        status: "Success",
                        message:
                          "User registered and order created successfully!Please Make Payment",
                        userId,
                        data: response.data,
                      });
                    } else {
  ...
  try {
    ...
    if (response.data.status === "Success") {
      ...
      if (dbData.payment_status === "Pending") {
        ...
        await new Promise((resolve, reject) => {
          db.query(
            "UPDATE userprofile SET payment_status = ? WHERE UserId = ?",
            ["Complete", clientTxnId],
            (err) => {
              if (err) reject(err);
              resolve();
            }
...
const MakePaymentINPortal = async (req, res) => {
  ...
  const { userId, website, total_amount, payment_method, userPhone } = req.body;

  // Validate the request data
  if (!userId || !website || !total_amount || !payment_method || !userPhone) {
    return res.status(400).json({ message: "All fields are required." });
  }

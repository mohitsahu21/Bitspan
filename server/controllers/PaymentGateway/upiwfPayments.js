...
const webhook = (req, res) => {
  ...
  console.log(clientTxnId)
    // const key = '4654407eb1b965763bcfcb9ae8cc87ec';
    const key = '87fe0b14e71fee2766e4bde07bcc1949';
    ...
    try {
      ...
      const response = await axios.post('https://qrpay.index.wf/api/check-order-status', postData.toString(), { headers: { 'Content-Type': 'application/x-www-form-urlencoded' } });
      console.log(response)
      if (response.data.status === 'Success') {
        const txnStatus = response.data.result.txnStatus;
        const amount = response.data.result.amount;
    ...
    try {
      ...
      try {
        const postData = { customer_mobile,
          ...
          order_id,
          redirect_url};
        const response = await axios.post('https://qrpay.index.wf/api/create-order',
          postData
        );
        console.log(response);
  ...
  const addWalletMoneyUsingPG = async (req, res) => {
    ...
    try {
      ...
      const postData = new URLSearchParams({ user_token: key, order_id: dbData.order_id });
      const response = await axios.post('https://qrpay.index.wf/api/check-order-status', postData.toString(), {
  ...
  `,
                        ...
                        transporter.sendMail(mailOptions, async (emailErr) => {
                            ...
                            try {
                                ...
                                const response = await axios.post('https://qrpay.index.wf/api/create-order', postData);

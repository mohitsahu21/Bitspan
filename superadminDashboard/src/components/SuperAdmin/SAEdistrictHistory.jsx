  ...
  const handlesubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        "https://2kadam.co.in/api/auth/superAdmin/ApproveEdistrictForm",
...
const SAMarkEditModel = ({ item, setShowMarkEditModel, setIsRefresh }) => {
  ...
  const handlesubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const response = await axios.put(
        "https://2kadam.co.in/api/auth/superAdmin/markForEditEdistrictForm",
        // "https://2kadam.co.in/api/auth/superAdmin/resolveComplaint",
        formData,
        {
          headers: {
...
const SASuccessModel = ({ item, setShowSuccessModel, setIsRefresh }) => {
  ...
  //     setLoading(true);
  //     const response = await axios.put(
  //       "https://2kadam.co.in/api/auth/superAdmin/SuccessEdistrictForm",
  //       // "https://2kadam.co.in/api/auth/superAdmin/resolveComplaint",
  //       formData,
  //       {
  //         headers: {
  ...
  //   }
  // };
  const EdistrictCommission = async (retailer, distributor, superDistributor, white_lable ,packageDetails,item)=>{
    const Order_Id = `ORW${Date.now()}`;
    const Transaction_Id = `TXNW${Date.now()}`;
   const  retailerFormData = {
      userId: retailer,
  amount : "",
  Transaction_details : `Commission Credit for Editstrict Application Order Id ${item.order_id}`,
  status : "Success",
  Order_Id,
  Transaction_Id
    }
    ...
    try {
          ...
          if(retailerPackage.Offline_Services_Commission_Type == "Percentage"){
            if(application_type == "income"){
              retailerCommAmount = (amount * parseFloat(retailerPackage.eKYC_Income_Certificate_Commission))/100
            }
            else{
              retailerCommAmount = (amount * parseFloat(retailerPackage.eKYC_Domicile_Certificate_Commission))/100
            }
              ...
              if(application_type == "income"){
                retailerCommAmount = parseFloat(retailerPackage.eKYC_Income_Certificate_Commission);
              }
              else{
                retailerCommAmount = parseFloat(retailerPackage.eKYC_Domicile_Certificate_Commission)
              }
          ...
          if(distributor && distributorPackage){
            if(distributorPackage.Offline_Services_Commission_Type == "Percentage"){
                ...
                if(application_type == "income"){
                  distributorCommAmount = (amount * parseFloat(distributorPackage.eKYC_Income_Certificate_Commission))/100
                }
                else{
                  distributorCommAmount = (amount * parseFloat(distributorPackage.eKYC_Domicile_Certificate_Commission))/100
                }
                ...
                if(application_type == "income"){
                  distributorCommAmount = parseFloat(distributorPackage.eKYC_Income_Certificate_Commission);
                }
                else{
                  distributorCommAmount = parseFloat(distributorPackage.eKYC_Domicile_Certificate_Commission)
                }
            }
          ...
          if(superDistributor && superDistributorPackage){
            if(superDistributorPackage.Offline_Services_Commission_Type == "Percentage"){
                ...
                if(application_type == "income"){
                  superDistributorCommAmount = (amount * parseFloat(superDistributorPackage.eKYC_Income_Certificate_Commission))/100
                }
                else{
                  superDistributorCommAmount = (amount * parseFloat(superDistributorPackage.eKYC_Domicile_Certificate_Commission))/100
                }
                ...
                if(application_type == "income"){
                  superDistributorCommAmount = parseFloat(superDistributorPackage.eKYC_Income_Certificate_Commission);
                }
                else{
                  superDistributorCommAmount = parseFloat(superDistributorPackage.eKYC_Domicile_Certificate_Commission)
                }
            }
          ...
          if(white_lable && whiteLablePackage){
            if(whiteLablePackage.Offline_Services_Commission_Type == "Percentage"){
                ...
                if(application_type == "income"){
                  whiteLableCommAmount = (amount * parseFloat(whiteLablePackage.eKYC_Income_Certificate_Commission))/100
                }
                else{
                  whiteLableCommAmount = (amount * parseFloat(whiteLablePackage.eKYC_Domicile_Certificate_Commission))/100
                }
            }
            else{
                ...
                if(application_type == "income"){
                  whiteLableCommAmount = parseFloat(whiteLablePackage.eKYC_Income_Certificate_Commission);
                }
                else{
                  whiteLableCommAmount = parseFloat(whiteLablePackage.eKYC_Domicile_Certificate_Commission)
                }
            }
    ...
    try {
      ...
      const retailerPromise = axios
        ...
        // Use the results as needed
          if(item.samagar == "ekyc"){
            result = await EdistrictCommission(retailer, distributor, superDistributor, white_lable ,resultsMap,item)
            console.log(result)
          }
...
const SARejectModel = ({ item, setShowRejectModel, setIsRefresh }) => {
  ...
  const handlesubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const response = await axios.put(
        "https://2kadam.co.in/api/auth/superAdmin/rejectEdistrictForm",
        // "https://2kadam.co.in/api/auth/superAdmin/resolveComplaint",
        formData,
        {
          headers: {
...
const SAEdistrictHistory = () => {
  ...
  const fetchOfflineForm = async () => {
    ...
    try {
      const { data } = await axios.get(
        "https://2kadam.co.in/api/auth/superAdmin/getEdistrictForms",
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
  ...
  const filteredItems = users.filter((row) => {
    const matchesKeyword =
      ...
      (row?.aadhar_no &&
        row.aadhar_no.toLowerCase().includes(keyword.trim().toLowerCase())) ||
      (row?.order_id &&
...
export default SAEdistrictHistory;

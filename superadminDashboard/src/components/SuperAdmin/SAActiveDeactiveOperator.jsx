...
const SAActiveDeactiveOperator = () => {
  const [loading, setLoading] = useState(false);
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const { token } = useSelector((state) => state.user);
  ...
  const filteredItems = users.filter((row) => {
    const matchesKeyword =
      ...
      (row?.category &&
        row.category.toLowerCase().includes(keyword.trim().toLowerCase())) ||
      (row?.operator_name &&
        row.operator_name.toLowerCase().includes(keyword.trim().toLowerCase()));
                        ...
                        <div className="d-flex flex-column flex-xl-row gap-3">
                          <div className="col-12 col-md-12 col-lg-12 col-xl-8">
                            ...
                            <input
                              ...
                              type="search"
                              placeholder="Enter API Name/Operator Name/Category"
                              value={keyword}
                              onChange={(e) => {
        ...
        >
          ...
          <Modal.Body>
            {selectedComplaint && (
              <SADeactiveApi
                complaint={selectedComplaint}
                setShowDeactiveModel={setShowDeactiveModel}
                setIsRefresh={setIsRefresh}
              />
            )}
          </Modal.Body>
...
export default SAActiveDeactiveOperator;

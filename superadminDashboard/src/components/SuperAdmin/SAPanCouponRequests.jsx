...
const SAApproveModel = ({ item, setShowApproveModel, setIsRefresh }) => {
 ...
 const { token } = useSelector((state) => state.user);
  ...
  const CouponCommission = async (retailer, distributor, superDistributor, white_lable ,packageDetails,item)=>{
    const Order_Id = `ORW${Date.now()}`;
    const Transaction_Id = `TXNW${Date.now()}`;
   const  retailerFormData = {
      userId: retailer,
  amount : "",
  Transaction_details : `Commission Credit for Pan Card Coupon Order Id ${item.order_id}`,
  status : "Success",
  Order_Id,
  Transaction_Id
    }
  ...
  Transaction_Id
    ...
    try {
          ...
          if(retailerPackage.PAN_Card_Commission_Type == "Percentage"){
              ...
              retailerCommAmount = (amount * parseFloat(retailerPackage.UTI_PAN_Coupon_Commission))/100
            
           
          }
          else{
             
                retailerCommAmount = (quantity * parseFloat(retailerPackage.UTI_PAN_Coupon_Commission));
          ...
          if(distributor && distributorPackage){
            if(distributorPackage.PAN_Card_Commission_Type == "Percentage"){
                  ...
                  distributorCommAmount = (amount * parseFloat(distributorPackage.UTI_PAN_Coupon_Commission))/100
            }
            else{
              
                  distributorCommAmount = (quantity * parseFloat(distributorPackage.UTI_PAN_Coupon_Commission));
          ...
          if(superDistributor && superDistributorPackage){
            if(superDistributorPackage.PAN_Card_Commission_Type == "Percentage"){
                  ...
                  superDistributorCommAmount = (amount * parseFloat(superDistributorPackage.UTI_PAN_Coupon_Commission))/100
                
            }
            else{
              
                  superDistributorCommAmount = (quantity * parseFloat(superDistributorPackage.UTI_PAN_Coupon_Commission));
          ...
          if(white_lable && whiteLablePackage){
            if(whiteLablePackage.PAN_Card_Commission_Type == "Percentage"){
                  ...
                  whiteLableCommAmount = (amount * parseFloat(whiteLablePackage.UTI_PAN_Coupon_Commission))/100
              
            }
            else{
              
                  whiteLableCommAmount = (quantity *  parseFloat(whiteLablePackage.UTI_PAN_Coupon_Commission));
    ...
    try {
      ...
      const retailerPromise = axios
        ...
        // Use the results as needed
            ...
            result = await CouponCommission(retailer, distributor, superDistributor, white_lable ,resultsMap,item)
            console.log(result)

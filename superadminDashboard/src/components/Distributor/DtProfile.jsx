  ...
  const fetchUserData = async () => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:7777/api/auth/Distributor/getUserDetails/${currentUser?.userId}`,
        {
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:7777/api/auth/Distributor/user-profile/${currentUser?.userId}`,
        formData,
        {
          headers: {
      ...
      Swal.fire({
        title: "Success",
        text: response.data.message || "Upload successful!",
        icon: "success",
        confirmButtonText: "OK",
        // }).then(() => {
        //   // Update status to 'Active' after SweetAlert confirmation
        //   setStatus("Active");
      });
    } catch (error) {

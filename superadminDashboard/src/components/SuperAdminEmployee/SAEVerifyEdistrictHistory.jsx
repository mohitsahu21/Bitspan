    ...
    try {
      if (retailerPackage.Offline_Services_Commission_Type == "Percentage") {
        retailerCommAmount =
          (amount *
            parseFloat(
              retailerPackage.verify_edistrict_Certificate_Commission
            )) /
          100;
      } else {
      ...
      if (distributor && distributorPackage) {
        if (
          ...
          distributorCommAmount =
            (amount *
              parseFloat(
                distributorPackage.verify_edistrict_Certificate_Commission
              )) /
            100;
        } else {
      ...
      if (superDistributor && superDistributorPackage) {
        if (
          ...
          superDistributorCommAmount =
            (amount *
              parseFloat(
                superDistributorPackage.verify_edistrict_Certificate_Commission
              )) /
            100;
        } else {
      ...
      if (white_lable && whiteLablePackage) {
        if (
          ...
          whiteLableCommAmount =
            (amount *
              parseFloat(
                whiteLablePackage.verify_edistrict_Certificate_Commission
              )) /
            100;
        } else {
...
const SAEVerifyEdistrictHistory = () => {
  const [loading, setLoading] = useState(false);
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const { token } = useSelector((state) => state.user);
...
export default SAEVerifyEdistrictHistory;

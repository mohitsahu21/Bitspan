  ...
  const fetchBankAccounts = async () => {
    try {
      const response = await axios.get(
        `http://localhost:7777/api/auth/superDistributor/getActiveBankDetails/${userId}`,
        {
          headers: {
  ...
  const submitHandler = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `http://localhost:7777/api/auth/superDistributor/WalletWithdraw/${userId}`,
        formData,
        {
          headers: {
            "Content-Type": "application/json",
...
export default SdWalletWithdraw;

...
const SAHistoryDSC = () => {
  ...
  const fetchRechargeData = async () => {
    ...
    try {
      const response = await axios.get(
        ...
        `https://2kadam.co.in/api/auth/superDistributor/getDSCHistory/${userID}`,
        {
          headers: {
  ...
  const filteredItems = apiData.filter((row) => {
    ...
    const matchesDate =
      (!fromDate ||
        new Date(row.create_at).toISOString().split("T")[0] >=
                                  ...
                                  {showApiData && showApiData.length > 0 ? (
                                    showApiData.map((item, index) => (
                                      <tr key={index}>
                                        ...
                                        <td>{item.plan}</td>
                                        <td>
                                          {maskSensitiveInfo(item.mobile, 6, 4)}
                                        </td>
                                        <td>{item.status}</td>
                                      </tr>
                                    ))
